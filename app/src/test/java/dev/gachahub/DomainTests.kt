...
class DomainTests {
    ...
    @Test fun zzzImport() {
        val input="""{"PlayerInfo":{"ShowcaseDetail":{"AvatarList":[{"Id":1191,"Level":60,"PromotionLevel":6,"TalentLevel":3,"CoreSkillEnhancement":2,"Weapon":{"Id":14119,"Level":60,"UpgradeLevel":1},"SkillLevelList":[{"Index":0,"Level":10}],"EquippedList":[{"Slot":4,"Equipment":{"Id":123,"Level":15}}]}]}},"ttl":60}"""
        val result=ShowcaseParser.parse(Game.ZZZ,input,emptyList(),1)
        assertEquals("zzz:1191",result.owned.single().characterId);assertEquals(10,result.owned.single().skills["Basic"]);assertEquals(2,result.owned.single().skills["Core"])
